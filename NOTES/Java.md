...
CopyOnWriteArrayList 写时复制, 向列表中添加元素的时候就要重新生成一个容器用来存储列表元素, 将元素添加到新的列表中去, 然后将原容器的引用指向新的容器. 适合读多写少的场景, 但是写入的时候不允许多个线程写入, 防止多个线程生成过多的容器, 导致内存消耗尽. 运行在写入的过程中读取, 此时读取到的是原容器的元素.
...
分布式锁, 是分布式系统中互斥访问共享资源的一种手段, 防止并行访问引发的结果不可控(例如商品超卖等). 其实现原理和单进程锁是相同的, 都是通过一个共享标识来确定唯一性, 对共享标识进行修改时能够保证原子性和对锁服务调用方的可见性.
...
掘金博客: https://juejin.cn/post/6844903744954433544
...
1.使用new关键字创建实例时, 对应的类必须初始化

2.获取或者设置类的静态变量(对应getstatic, putstatic指令)(静态变量被final修饰, 且在编译期就放入常量池的静态字段除外), 

调用类的者类的静态方法时(对应invokestatic指令), 对应的类必须初始化

3.使用反射java.lang.reflect包下对应的方法对类进行反射调用的时候, 类必须初始化

4.子类初始化的时候, 如果父类还没有初始化必须触发父类的初始化(接口的父接口不需要遵循这个规则)

5.使用JDK 8, 接口(interface)中定义了默认方法, 该接口的实现类初始化时, 接口必须要先初始化

6.如果使用JDK 7中的动态语言支持时, 如果一个java.lang.invoke.MethodHandle 实例最后的解析结果为

REF_getStatic, REF_putStatic, REF_invokeStatic, REF_newInvokeSpecial四种类型的方法句柄, 这个方法句柄对应的类必须初始化

除去上述6种情况, 其他情况都不强制类初始化, 如下:
