...
MVCC，多版本并发控制，采用了多版本并发控制来提高读操作的性能，MVCC 提供了非锁定读：不需要等待访问行上的锁释放，读取行的一个快照。只有 READ_COMMITTED 和 REPEATABLE_READ 两种事务隔离级别才能使用 MVCC。MVCC是通过undo log实现, undo log虽然叫做log, 其实是数据的备份, 同一条数据记录的不同版本的备份, 通过列表连接。
