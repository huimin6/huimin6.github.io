...
InnoDB中的trx_id是通过一个全局变量值max_trx_id来控制的, 这个值会一直单调递增, 最大值是(2^48)-1. 所以这个值还是会用尽, 当这个值达到(2^48)-1时就会从0重新开始, 

会导致数据库的脏读问题(新创建的事务id总是能被这个最大值之前的事务读取到). 但是这个值达到最大值, 需要保持每秒50万TPS, 且持续允许17.8年才能达到, 所以一般不用担心这个值
