...
A，Atomic，原子性，就是指事务中的操作时一个整体，不可拆分, 通过undo log来保障

C，Consistency，一致性，就是指事务提交完成后，数据库中的数据要保持操作前后的一致性, 通过redo log和binlog来保障

I，Isolation，隔离性，不同的事务在并发操作的时候互不干扰

D，Durability，持久性，就是事务提交完毕，操作记录会被持久存储在数据库中, 通过redo log来保障
...
(3) 幻读，就是两个事务同时对数据库中的表操作，事务一统计表中的数据记录，事务二向表中插入一条记录后提交，事务一两次统计的结果不一样。简单说就是 A 事务读取了 B 事务提交的新增数据, 幻读是数据条数发生了变化, 不可重复读是数据内容发生了变化
...
| 保护内容 | 数据库内容 | 内存中的数据结构 |
| 持续时间 | 整个事务过程 | 临界资源 |
| 模式 | 行锁 表锁 意向锁 | 读写锁rwlock 互斥量mutex |
| 死锁 | 通过wait-for graph, time out等机制进行死锁检查和处理 | 无死锁检查和处理机制, 仅通过应用程序加锁的顺序保障无死锁情况发生 |
| 存在于 | Lock Manager的哈希表中 | 每个数据结构的对象中 |

# 间隙锁

grap lock, mysql默认的RepeatableRead本身不能解决幻读的问题, 是通过间隙锁解决的幻读的问题

间隙锁锁定的范围和字段 是否有索引, 索引类型有关系, 唯一索引和主键索引会优化为行锁, 普通索引会锁定范围, 没有索引会锁定全部
...
MVCC，多版本并发控制，采用了多版本并发控制来提高读操作的性能，MVCC 提供了非锁定读：不需要等待访问行上的锁释放，读取行的一个快照。只有 READ_COMMITTED 和 REPEATABLE_READ 两种事务隔离级别才能使用 MVCC。MVCC是通过undo log实现, undo log虽然叫做log, 其实是数据的备份, 同一条数据记录的不同版本的备份, 不同版本的数据通过列表连接。
...
InnoDB中的trx_id是通过一个全局变量值max_trx_id来控制的, 这个值会一直单调递增, 最大值是(2^48)-1. 所以这个值还是会用尽, 当这个值达到(2^48)-1时就会从0重新开始, 导致数据库的脏读问题(新创建的事务id总是能被这个最大值之前的事务读取到). 但是这个值达到最大值, 需要保持每秒50万TPS, 且持续允许17.8年才能达到, 所以一般不用担心这个值用尽的问题
...
https://learnku.com/articles/38615
